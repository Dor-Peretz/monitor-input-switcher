#!/usr/bin/env python3
"""Toggle monitor inputs with global hotkeys (DDC/CI on Windows)."""
...
from ddc_monitor import enumerate_monitors, print_monitor_list
from hotkey_service import HotkeyService, parse_input_value, run_toggle_cli


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Switch monitor inputs using DDC/CI and global hotkeys."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command == "init":
        ...
        print("Run 'py -3 ui.py' to configure hotkeys in the UI.")
        return 0
    ...
    if command == "toggle":
        ...
        if args.inputs:
            ...
            parser.error(
                "Provide --inputs A B or add a binding for this monitor in config.json."
            )
